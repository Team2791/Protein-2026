...
public class ModuleIOSpark implements ModuleIO {
    ...
    public void updateInputs(ModuleIOInputs inputs) {
        ...
        ifOk(driveSpark, driveEncoder::getPosition, value ->
            inputs.drivePositionRad = value
        );
        ifOk(driveSpark, driveEncoder::getVelocity, value ->
            inputs.driveVelocityRadPerSec = value
        );
        ifOk(
            driveSpark,
            new DoubleSupplier[] {
        ...
        // Update turn inputs
        sparkStickyFault = false;
        ifOk(turnSpark, turnEncoder::getPosition, value ->
            inputs.turnPosition = new Rotation2d(value).minus(zeroRotation)
        );
        ifOk(turnSpark, turnEncoder::getVelocity, value ->
            inputs.turnVelocityRadPerSec = value
        );
        ifOk(
            turnSpark,
            new DoubleSupplier[] {
